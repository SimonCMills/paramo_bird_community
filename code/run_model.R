# fit models
library(dplyr); library(flocker); library(mgcv)

# datasets
bird_data <- readRDS("data/paramo_bird_dataset.rds")
veg_data <- bird_data %>%
    select(point, elev_ALOS, habitat_sc, habitat_type, site, cluster, 
...
cov <- select(bird_data, species, phylo = species_eltontraits, 
              abu_espeletia, abu_shrub, 
